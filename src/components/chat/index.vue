<template>
  <div class="chat-wrap" @click='boxClick'>
    <div class="announce-box" v-if="announceShow">
      <svg class="icon" style="width: 6.667vw; height: 6.667vw;vertical-align: middle;fill: currentColor;overflow: hidden;" viewBox="0 0 1305 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="3285">
    ...
    <div class="bscroll" ref="bscroll" :class="type !== 'live'? 'vod' : 'live'" @mouseover="stopScroll = true" @mouseout="stopScroll = false" @scroll="scrollEvent($event)">
      <ol class='chat-list bscroll-container'>
        <li v-for='(item,idx) in chatData' :data-joinId="item.id" :class="{'right': (joinInfo.consumerUserId ? joinInfo.consumerUserId : joinInfo.visitId) == item.id}" :key="idx">
...
<template v-if='swapAnnounce'>
  <com-input :value.sync="value" :placeholder="chatPlaceholder" :max-length="140" class='inp' :isCharacter=true type="textarea">
  </com-input>
</template>

<template v-else>
  <com-input :value.sync="valueAnnounce" :placeholder="announcePlaceholder" :max-length="140" class='inp' :isCharacter=true type="textarea">
  </com-input>
</template>
          <div class="v-emoji">
